package com.cmlx.commons.springExtension.resolver;

import com.cmlx.commons.springExtension.resolver.annotation.MapWapper;
import com.cmlx.commons.springExtension.resolver.annotation.RequestModel;
 ...
 * @Desc ->
 **/
public class RequestModelMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    protected void bindRequestParameters(ModelAndViewContainer mavContainer, WebDataBinderFactory binderFactory, WebDataBinder binder, NativeWebRequest request, MethodParameter parameter)
        ...
        if (Collection.class.isAssignableFrom(targetType)) {// bind collection
            ...
            Set<String> prefixNameRecord = new HashSet<>();

            for (Object key : servletRequest.getParameterMap().keySet()) {
                String prefixName = getPrefixName((String) key);
                boolean flag = prefixNameRecord.contains(prefixName);
                prefixNameRecord.add(prefixName);
    ...
    private ServletRequest prepareServletRequest(Object target, NativeWebRequest request, MethodParameter parameter) {
        ...
        String modelPrefixName = parameter.getParameterAnnotation(RequestModel.class).value();

        HttpServletRequest nativeRequest = (HttpServletRequest) request.getNativeRequest();
        MultipartRequest multipartRequest = WebUtils.getNativeRequest(nativeRequest, MultipartRequest.class);

        // update by xieyj on 2017-09-01 start       实例化请求
        ...
        // update by xieyj on 2017-09-01 end
