...
public class EmqClient {
    ...
    public void connect(String username, String password) {
        ...
        /**
         * 设置为false：表明客户端和服务端创建的是持久的会话，如果客户端断开连接，但是会话会一直保持，
         * 对于服务端来说认为客户端还在，如果需要推送数据还是会继续推送数据，但是这个消息会是一个离线消息
         * 客户端断开连接后emq还保留会话的话，会产生订阅共享队列的存活，客户端收不到消息的情况
         * 因为断开的连接还被保留的话，emq会将队列中的消息负载到断开但还保留的客户端，导致存活的客户端收不到消息
     ...
     * @param retain
     */
    public void publish(String topic, String msg, Constant.QosEnum qos, boolean retain) {
     ...
     * @param qos
     */
    public void subscribe(String topicFilter, Constant.QosEnum qos) {
        try {
